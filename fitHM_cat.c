void fitHM_cat(int year=2018){
	...
	/* const int nstage=1; */
	/* TString stageName [nstage]={"ggH"}; */
	/* int stage [nstage]= {0}; */

	const int nstage=1;
	TString stageName [nstage]={"VBF_2jets"};
	int stage [nstage]= {1};
	...
	float a1_DCB [6][21]={
	  1.2419,1.15,1.2,1.2194,1.3056,1.1834,1.3413,1.3367,1.2401,1.2318,1.3577,1.1751,1.1597,1.1928,1.2527,1.1341,1.2005,1.2003,1.2233,1.2122,1.2064, //4mu ggH
	  ...
	  1.0,1.1,1.2,1.2470,1.2669,1.3139,1.2892,1.2717,1.1509,1.1685,1.1076,1.2109,1.2654,1.3116,1.3301,1.0984,1.0898,1.0002,1.0367,1.2,1.4186, //4mu VBFH
	...
	float a2_DCB [6][21]={
	  ...
	  1.5,1.6,1.7,1.9183,1.8129,1.8694,1.6360,1.5863,1.0010,1.0129,1.0608,1.5351,1.1812,2.0776,1.4164,1.2640,1.2893,1.0035,1.0106,1.2,1.4869,
	  1.5,1.7,1.7,1.9585,1.8763,1.7642,1.4552,1.5432,1.3811,1.4278,1.4475,1.5333,1.4776,1.5569,1.3670,1.4236,1.3001,1.4630,1.4887,1.47,1.4653,
	  1.5,1.6,1.7,1.7349,1.5972,1.5839,1.5865,1.4639,1.4984,1.4808,1.4942,1.4835,1.3565,1.2604,1.2841,1.2649,1.3415,1.2723,1.2403,1.3,1.3707
	};
	float n1_DCB [6][21]={
	  ...
	  1.4568,2.3,2.4,2.5282,2.3005,2.2804,2.0976,2.3546,1.8135,2.1955,2.9664,1.7682,1.9978,2.0053,1.7789,1.7352,1.7147,1.8324,1.2500,1.1,1.0010,
	  1.6264,1.7,1.8,1.8120,2.1907,2.3989,2.3008,2.2780,2.4135,2.7077,2.0415,2.3285,2.2460,2.1987,2.2848,2.3302,2.3305,2.6363,2.2268,1.9,1.5629,
	  ...
	  1.7,2.4,2.6,2.7588,1.4158,2.9465,2.3930,2.1092,3.1218,2.4703,2.7062,1.8016,2.1265,2.3799,2.0897,2.6818,2.5226,2.4442,2.3063,2.0,1.4456,
	  2.5,2.3,1.9,1.9433,2.0555,2.1605,2.1170,2.7849,2.4604,2.2141,2.7054,2.0967,2.4989,2.3064,2.5088,2.9055,2.8528,2.5623,4.5470,3.2,2.5025
	};
	float n2_DCB [6][21]={
	  1.375,2.3,2.5,2.7300,3.1547,3.9043,3.7195,4.1315,5.6250,4.3445,5.7724,7.5556,9.8253,9.9259,9.6037,7.1887,6.9848,5.7123,3.9931,2.3,1.9656,
	  1.4376,2.5,2.8,3.0247,5.8120,4.7525,5.7845,6.1304,4.8972,7.8430,7.9764,6.7244,7.0848,6.7780,7.1157,6.9672,5.5329,5.2339,4.6659,4.7,4.7416,
	  1.7589,2.5,3.,3.7065,3.5213,4.7145,5.1935,5.0584,5.0224,5.7709,4.7919,7.3961,6.5437,6.7708,7.5857,7.2731,6.2044,6.5999,5.2371,3.5,2.7961,
	  2.4,2.2,2.0,1.7967,2.4162,2.4376,3.5257,3.5147,4.1033,5.2645,7.3337,7.5293,6.5710,6.0014,4.8617,3.9502,3.7235,3.7503,3.9706,4.0,4.0411,
	  2.5,2.3,2.1,1.4292,2.3610,2.6239,5.3727,4.1560,6.0071,5.5596,4.9926,5.6982,5.5362,4.5224,6.3557,5.8163,7.2659,5.6072,5.0098,5.2,5.3683,
	  2.5,2.3,2.1,2.1439,3.1216,3.4119,3.6354,4.3947,4.3994,5.6987,6.9944,7.6535,8.8642,8.9118,8.5872,7.2481,5.6786,5.0373,5.1350,5.15,5.2018
	};
	float mean_DCB [6][21]={
	...
	for (int j =0;j<3;j++){
		...
		for (int i =0;i<N;i++){
		  ...
		  //    cout << sigName.at(l) << " added to TChain" << endl;
			...
			data[i]=new RooDataSet(Form("data%d_%d",i,mass[i]),"",t,RooArgSet(*ZZMassCorr[i],*weight,*chan,*vbfcate),Form("chan==%d",j+1),"weight");
		}

		for (int k =0;k<nstage;k++){
			//	MH->setVal(mass[i]);
			//	MH->setConstant(kTRUE);
		        cout<<"stageName "<<stageName[k].Data()<<endl;
		        outmass.open(Form(path + "shapeParamCategory/sim_massParam_Cat_%s_%s_%d.txt",stageName[k].Data(),chanName[j].Data(),year));
			...
			for (int i=0;i<N;i++){
				...
				RooRealVar *a1_0_mh=new RooRealVar("a1_0_mh","",a1_DCB[k*3+j][i2],0.,5.);
				...
				RooRealVar *a2_0_mh=new RooRealVar("a2_0_mh","",a2_DCB[k*3+j][i2],0.,5.);
				...
				data_stage_cat[i] = new RooDataSet(Form("data_%d",i),"",RooArgSet(*ZZMassCorr[i],*weight),Index(massrc),Import(cat_type,*data_stage[i]),WeightVar("weight")); 
				cpdf[i] = new RooDoubleCBFast(Form("DCBall_%d",i),"",*ZZMassCorr[i],*mean,*sigma,*a1,*n1,*a2,*n2);
				...
				//RooDoubleCBFast *cpdftmp = new RooDoubleCBFast("DCBall","",*ZZMassCorr[i],*mean,*sigma,*a1,*n1,*a2,*n2);
					...
					//   cc     addpdftmp->fitTo(*data_stage_cat[i],InitialHesse(true),Strategy(2));
				        //else
				cpdf[i]->fitTo(*data_stage_cat[i],InitialHesse(true),Strategy(2));
				cpdf[i]->Print("v");
				data_stage_cat[i]->Print("v");
...
g			     TString a2outs= Form ("a2_%s_%s_%d_%d \t %.4f",stageName[k].Data(),chanName[j].Data(),year,mass[i],a2_0_mh->getVal());
			...
			cout<<"where1"<<endl;
			for (int i =0;i<N;i++){
			        RooPlot *frame = ZZMassCorr[i]->frame();
				TString cat_type =Form("mh%d",mass[i]);
				data_stage_cat[i]->plotOn(frame,Cut(Form("massrc==massrc::mh%d",mass[i]))) ;
				cpdf[i]->plotOn(frame,Slice(massrc,cat_type),ProjWData(massrc,*data_stage_cat[i])) ;
				frame->chiSquare();
				frame->residHist();
